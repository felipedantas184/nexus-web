// Shared pure functions for weekly metric calculation
// Duplicated from lib/utils/weeklyMetrics.ts because cloud-functions is a separate npm package
...
export function calculateWeeklyMetrics(activities: ActivityData[]): WeeklyMetrics {
  return {
    totalActivities: calculateTotalActivities(activities),
    completedActivities: calculateCompletedActivities(activities),
    skippedActivities: calculateSkippedActivities(activities),
    completionRate: calculateCompletionRate(activities),
    totalPointsEarned: calculateTotalPoints(activities),
    averagePointsPerActivity: calculateAveragePointsPerActivity(activities),
    totalTimeSpent: calculateTotalTimeSpent(activities),
    averageTimePerActivity: calculateAverageTimePerActivity(activities),
    consistencyScore: calculateConsistencyScore(activities),
    adherenceScore: calculateAdherenceScore(activities),
    streakAtEndOfWeek: calculateStreakAtEndOfWeek(activities),
  };
}
